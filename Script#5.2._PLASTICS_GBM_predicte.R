...
##### 27/01/23: R script to examine Kevin Lang's legacy (data, codes etc.), © Fabio Benedetti, UP group, IPB, D-USYS, ETH Zürich. 
...
#   - Train 1000 GBM models per GNI class by using all potential PV set based on the parameters defined in Script#5.1
#   - Predict annual MSW per country usinng either all 1000 GBMs (ensemble mean) 
...
### Write master function to train 1000 GBMs per GNI class and get predictins and errors to un-transformed MSW obs.

### 30/01/23: Fixed typo in the extartcion of predictions for non missing countries (@ line 376)
# inc <- "H"
gbm.predicter <- function(inc) {
        
        # Parameters chosen based on results from Script5.1
        if(inc == "H") {
        ...
        mclapply(c(1:l), function(i) {
                ...
                res <- lapply(c(1:N), function(n) {                    
                            ...
                            gbm.mod <- gbm(formula = form, data = dat, distribution = "gaussian",
                            ...
                            fit <- predict(object = gbm.mod, newdata = dat[,preds], n.trees = Ntree_gbm)
                            ...
                            save(x = gbm.mod, file = paste("gbm.full_",inc,"_",paste(preds, collapse = '+'),"_",n,".Rdata", sep = "") )
                            ...
                            for( land in all.countries ) { 
                                      ...
                                      if( land %in% unique(dat$country) ) {
                                          ...
                                          fit.2 <- predict(object = gbm.mod, newdata = scaled.temp[,preds], n.trees = Ntree_gbm)
                                          # Extract predictions and errors - save in proper dir
                            ...
                            # Save outputs 
                            setwd("/net/kryo/work/fabioben/Inputs_plastics/2023/outputs/models/GBM_predictions")
...
ggsave(plot = plot, filename = "boxplot_full.gbms_R2_GNI_30_01_23.pdf", dpi = 300, height = 5, width = 4)
...
ggsave(plot = plot, filename = "boxplot_full.gbms_MSE_GNI_30_01_23.pdf", dpi = 300, height = 5, width = 4)
...
dim(m.ddf)
head(m.ddf)
summary(m.ddf$value) # from -84 to +322% and median error is -1.68% - not great compared to RF and GAMs 
sum.error <- data.frame(m.ddf %>% group_by(Country) %>% summarize(median = median(value)) ) # eo ddf
# summary(sum.error) 



### 27/01/23: Compute mean annual ensemble predictions for GBMs. Re-compute error of the ensemble predictions to the y_org. 
setwd("/net/kryo/work/fabioben/Inputs_plastics/2023/outputs/models/GBM_predictions")
...
write.table(x = ensemble, "table_ensemble_predictions_GBM_30_01_23.txt", sep = "\t")
...
### Save error table
write.table(error_ensemble, file = "table_GBM_median_errors_percentage_30_01_23.txt", sep = ";")

### And derive pluriannual mean of errors
m.error <- melt(error_ensemble, id.vars = "Country")
# summary(m.error$value) # from -64% to +214%, and median error is -1.80%
...
### Does not look too good! --> Compare to other models
